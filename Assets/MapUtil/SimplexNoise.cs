...
namespace Assets.MapGen
{
    class SimplexNoise
    {
        ...
        private byte[] permMod12 = new byte[512];


        private byte[] buildPermutationTable(Rander.RandFloat random)
        {
            int i;
            var p = new byte[256];
        ...
        public SimplexNoise(Rander.RandFloat rand)
        {
            p = buildPermutationTable(rand);
            for (int i = 0; i < 512; i++)
            {
                perm[i] = p[i & 255];
